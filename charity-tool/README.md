...
- **`app/charity_client.py`** — calls the four register endpoints, then normalises
  the (inconsistent) responses into one clean report:
  - `searchCharityName/{name}` — resolves a name to a registration number
...
- **`app/main.py`** — FastAPI routes + static UI.
...
- **Resolution rule.** All-digits input is treated as a registration number;
  anything else goes through name search, preferring active, non-subsidiary
  charities.
...
Commission's [full data extract](https://register-of-charities.charitycommission.gov.uk/en/download-charity-data)
(or the BigQuery/CharityBase mirrors) into a local table and query that, then
reuse this same analysis + UI layer to render the multi-charity results.
